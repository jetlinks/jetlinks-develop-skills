...
12. If the capability gap is complex, crosses multiple boundaries, has more than one plausible root cause, or has already survived one implementation attempt, pair with [`../jetlinks-systematic-solving/SKILL.md`](../jetlinks-systematic-solving/SKILL.md). Let it govern evidence, hypotheses, stagnation, and the validation matrix; keep this skill responsible for implementation conventions and no-hack red lines.
...
- Do not continue adding special conditions, fallbacks, retries, mocks, or compatibility branches after a root-cause hypothesis has failed once. Switch to `$jetlinks-systematic-solving`, record what the result falsified, and rebuild the system model before further edits.
