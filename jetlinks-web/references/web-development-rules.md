...
6. 编码前执行组件抽离与复用门禁：确认要改哪些 Vue/JSX/TSX 文件、每个文件职责边界、是否可能超过 300 行、是否已有组件 / hooks / utils / services 可复用；同时扫描目标目录的实现文件数量、稳定职责域和外部深引用。
7. 若文件预计超过 300 行、重复结构出现 2 次以上、请求或业务编排准备进入组件，先拆分到子组件、hook、service/api 或 util，再实现；若目标目录已有 20 个及以上实现文件、形成 3 个及以上稳定职责域，或拆分后仍会继续平铺，先按 [`directory-structure-rules.md`](directory-structure-rules.md) 完成目录职责审查与稳定导出设计。
...
- 不要把原型图中的说明文字、交互原理、占位备注或开发者导向文案渲染到最终页面；这不禁止在源码中为函数职责、关键业务逻辑和兼容边界添加代码注释。
- Vue/JSX/TSX 文件超过 300 行必须拆分，这是编码阶段门禁，不是交付后建议。
- 目录规模治理也是编码阶段门禁：达到文件数量、职责域或深引用触发条件后，不得继续以“最小改动”为理由新增平铺文件；先按职责分域，或记录符合当前项目约定的明确例外。
...
- 是否在新增文件前检查目标目录规模、职责域和外部深引用，并在触发门禁后先完成目录治理。
