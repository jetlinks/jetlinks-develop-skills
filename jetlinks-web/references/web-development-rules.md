...
6. 编码前执行组件抽离与复用门禁：读取 [`code-organization-rules.md`](code-organization-rules.md)，确认要改哪些 Vue/JSX/TSX 文件、每个文件职责边界、职责是否混杂、是否已有组件 / hooks / utils / services 可复用。
7. 若文件职责混杂、语义相同的结构需要复用、请求或业务编排准备进入展示组件，先按 [`code-organization-rules.md`](code-organization-rules.md) 的边界判断拆分到子组件、composable/hook、service/api 或 util，再实现；不为降行数而拆。
...
11. 交互方案档案默认强制：除非任务是局部调整（仅改一个表单字段 / 仅改一个筛选条件 / 仅改一个弹窗内容 / 仅样式与文案与组件 props 微调），任何新增页面、重构页面壳层、调整信息架构或调整主筛选 / 主列表 / 主详情承载方式都必须先按 [`page-pattern-decision-rules.md`](page-pattern-decision-rules.md) 完成页面分型并建立方案档案再实现，不允许默认回退到传统 CRUD 页面。若已知事实清楚且只有一个可信推荐方案，可以记录“默认采用推荐方案”并继续；若页面骨架或信息架构风险较高，或 2 个以上方案同样合理，先给出线框图 / 方案选项并等待确认再实现。
...
14. 方案选定后，按场景先映射 `@jetlinks-web/components` 的共享基础组件，再映射 `@jetlinks-web-core/components` 的项目级组件与封装；若项目级组件已承载稳定业务契约则优先使用，最后才决定是否新增局部实现。
15. 使用最小完整改动完成需求，优先沿用现有页面组合和模块边界。
16. 按质量与类型约束做交付前自检，补充可验证路径。
17. 先确认后端枚举字段契约：`EnumDict` / `I18nEnumDict` 通常返回 `{ value, text }`。用户可见处显示 `text`，提交 / 筛选 / 比较 / 状态色使用 `value`；详细规则见 [`enum-rendering-rules.md`](enum-rendering-rules.md)。
...
- 页面 / 容器组件只保留布局组合、方案级状态展示和命令分发；展示组件只通过 props / slots / 语义 emits 承载独立 UI；请求、参数组装、业务编排、复杂计算按 [`code-organization-rules.md`](code-organization-rules.md) 下沉到 composable/hook、service/api 或纯 util/mapper；交付前在最终回复中说明本次新建或修改文件的职责划分与抽离决策。
