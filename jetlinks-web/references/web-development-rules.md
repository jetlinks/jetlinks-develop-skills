...
6. 若业务意图、用户角色或关键交互不清晰，先向用户提问；若页面骨架或信息架构风险较高，先给出线框图或效果草图再实现。
7. 使用最小完整改动完成需求，优先沿用现有页面组合和模块边界。
8. 按质量与类型约束做交付前自检，补充可验证路径。

## 与 `$frontend-design` 协同

- 仅当任务包含页面美化、交互优化、层级梳理、状态页完善或体验打磨时，再考虑结合 `$frontend-design`。
- 结合前必须先从当前工作区提炼具体样式锚点：组件库、排版密度、间距尺度、色彩和状态色、圆角/阴影、按钮和表单反馈、表格操作区、弹窗/抽屉/Tabs 行为、图标和动效语气。
- 把 `$frontend-design` 当成“现有框架内的交互与视觉润色助手”，而不是重新定义品牌、字体、配色和页面骨架的重设计引擎。
