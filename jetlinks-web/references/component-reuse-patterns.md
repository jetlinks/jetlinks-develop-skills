...
整页约定仅在修改相应交互时应用，并核对相邻详情页与模块的实际模式：

- 单对象详情头部的名称 / 说明采用 inline 编辑，标签就近增删，状态用徽标与快捷动作切换；不以“编辑 → 基本信息整表单”替代轻量编辑，也不重写已有 InputEditable / Editable / FormItemEditable。
- 相同字段类型跨模块复用同一编辑控件、触发方式和保存反馈；默认 hover 提示、点击进入，Enter 保存、Esc 取消、blur 保存，遵循目标组件的实际契约。
- 资产详情的反向引用（被哪些 Agent / 项目 / 规则使用）放在主内容区，显示计数，由实时反向查询提供数据，空时隐藏整个区段，不使用右侧 sticky 区。
- 侧栏可折叠父子分组复用共享组件；折叠为仅图标时用 `v-if/v-else` 在分组和平铺间切换，active 样式沿项目既有约定。
- 顶栏 Tab 的根路由不重复渲染 PageHead；非根路由保留声明式 PageHead。
- 浮动层使用 z-index token，不写 `9999` / `1000` 等裸值。默认每页一个右下 FAB；确有同锚点附加浮动控件时按项目的 56px 偏移协议排列。
...
- 用户完成任务需要什么术语；开发集成者或运维人员可能需要字段 key、API 路径、Topic、命令 ID 或权限码。
- 用户做完这一步后下一步是什么。

先复用用户请求、已接受设计和当前页面语义；仅当剩余未知会改变文案含义或操作路径时才询问，不按未知项数量暂停。

### 不允许出现

- 把用户的**原始需求文本、prompt、内部任务描述、需求文档原文、开发故事卡描述**直接放进页面文案、说明卡、Tooltip、空态提示、帮助区域或弹窗描述。
- 开发视角口吻："本功能用于实现...""我们将提供...""这里展示用户希望的...""按需求实现 XX 能力"。
- 与目标用户任务无关的内部标识进入可见文案。技术配置、接入和排障界面可以准确展示用户需要的 API 路径、Topic、命令 ID 等契约信息；需要时说明其用途和操作。
...
新增或替换相关组件时确认以下实际涉及的事实；沿用未变组件的局部调整不重新填表：
