...
- 反传统后台感硬约束（编辑触发梯度、首屏不可叠加 + 一屏一主视觉锚点、状态先于字段 + 状态强调收口、信息密度目标、文案约束、弹窗不是唯一编辑路径、详情页头部摘要区基本信息直接 inline 编辑、编辑交互样式统一、详情页 10 条硬规则 + AI 味 7 条不要 + 反向引用做主区段、侧栏 active 态分档 + 折叠 8 条精美规则、顶级路由不渲染 PageHead、浮动元素 z-index token 化）
- 业务任务 → 模版路由表
