...
5. For visual changes, follow [`references/style-rules.md`](references/style-rules.md) and reuse `comm.less` variables/utilities before writing local styles.
6. Run quality and type checks with [`references/quality-and-type-rules.md`](references/quality-and-type-rules.md) before final output.
7. Pair with `$jetlinks-conventions` when naming/import/i18n consistency is needed, and with `$jetlinks-delivery` when commit or PR output is requested.
...
- Do not hardcode design values in components when `comm.less` variables or utilities can express the same style.
