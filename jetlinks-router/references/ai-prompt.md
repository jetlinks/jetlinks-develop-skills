...
5. 选择单 Agent / 多 Agent 执行拓扑
    ...
    - 复杂跨模块任务若包含多个有实质工作量且可独立拥有的工作流，由 `$agent-orchestration` 判断是否建立 `OrchestrationProgram`；它只编排阶段，不新增前端 / 后端特调模式。主智能体保留用户决策、问题模型、公共契约、调度、集成、验收和交付，不与活跃 worker 重复叶子实现。
    - 先并行定位独立边界，再由主智能体冻结跨切片契约；只有阶段依赖已满足、相关契约 revision 已冻结、write set 互斥且各切片验收可独立判定时才并行写，否则使用顺序 handoff。
    - 未知根因、公共契约、迁移、安全、并发与共享状态由主智能体保留所有权；弱模型在同一切片一次有效失败后升级或重构 Assignment Capsule，不用换措辞继续同级重试。
    - 默认委派深度 1、同时 1–2 个有界切片、只读并行优先、共享写串行；当前路由选择多 Agent 且宿主允许时必须立即真实 dispatch、收集并集成，不能只输出角色或提示词；宿主不支持 subagent 时记录阻断原因并按相同角色契约串行执行。
...
- 当前执行选择非 `SINGLE_OWNER` 且宿主允许时，必须有真实 dispatch receipt 和终态 Result Packet；编排建议不算执行。
