...
4. 以当前模块风格为准
    ...
    - 前端任务中的交互和视觉也一样：优先服从当前前端框架、组件库和页面模式，不把 `$frontend-design` 当成另起一套风格的理由。
    - 前端页面结构还必须服从真实业务流程和用户任务，不要因为实现方便就默认退回后台 CRUD 套壳。
    - 前端设计默认以 Ant Design / Ant Design Vue 体系为准，参考案例只能作为辅助，且必须优先选择相邻页面、同域实现或相似业务场景。
    - 前端用户可见标题、字段展示名、表格列头、按钮、提示和状态文案应统一走当前工作区 i18n 机制，不要在页面代码中四处写死；如本地封装支持默认值，可使用中文作为默认值。
    - 不要为了页面饱满度而添加无业务意义的统计数字、装饰卡片、假图表或无来源数据展示。
    - 线框图、效果图、交互原理说明只用于设计对齐；最终实现必须是终端用户界面，不能把这些开发者导向内容直接展示出来。
...
适用：
- Vue3 页面、弹窗、列表、详情改造
- 需要复用 `@jetlinks-web-core/@jetlinks-web` 组件、hooks、utils
- 需要判断前端目录落点、状态边界、类型与质量约束
- 需要在现有框架风格内优化交互细节、视觉层级、空/错/载态和微交互
- 需要参考相似业务案例，但不能照搬完全不同场景的交互设计
- 需要先确认目标用户、第一任务、成功标准、单对象/多对象判断，以及关键指标的数据来源；不清楚时先问用户
- 如果页面壳层或风格存在多个合理方案，先组合 `jetlinks-web-style` 让用户选择，不默认传统 CRUD 管理页
- 如果存在通用条件搜索且 workspace 已提供 `ConditionFilter` 及编码回显工具，默认优先用 `ConditionFilter` 作为搜索层；`ProSearch` 仅用于相邻页面稳定沿用或轻量固定筛选
- 只有明确是标准管理页时，才进入搜索区 + 表格 + 弹窗/抽屉这一套管理页组合判断
- 需要保证用户可见标题、字段展示名、列头、按钮和提示文案统一走当前工作区 i18n 机制，必要时再组合 `$jetlinks-conventions`
- 需要先分析业务路径，必要时先问用户或先给线框图/效果图确认页面结构
- 需要保证线框图中的说明标注不会进入最终用户界面
...
- 前端页面业务流复杂且结构未定
    - `$jetlinks-web`
    - `$frontend-design`
    - 先询问用户关键业务与角色；必要时先输出线框图或效果草图，再进入实现

- 前端页面需要参考相似业务并统一 Ant Design 风格
    - `$jetlinks-web`
    - `$frontend-design`
    - 先确认参考案例与当前业务的相似点，只借鉴适配的交互与信息结构，避免无意义数据展示
