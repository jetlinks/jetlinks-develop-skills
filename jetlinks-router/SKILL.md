...
5. On resume, let `$task-continuity` complete `RESUME_AUDIT`; when another relation is needed, continue from capsule symbol / flow anchors instead of scanning the repository again. If validation changes the failure signature, partition mixed failures and refresh state before another production patch
