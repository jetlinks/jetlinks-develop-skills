...
9. When the task has independent read-heavy slices, a stable disjoint implementation slice, or a material review boundary, load [`../agent-orchestration/SKILL.md`](../agent-orchestration/SKILL.md). If a complex cross-module task contains multiple substantial independently ownable workstreams, let that skill consider an `OrchestrationProgram`: the primary remains the orchestrator, shared-contract owner, integrator, acceptance owner, and delivery owner while the current stage receives its own `RouteDecision`. Route by uncertainty, impact, coupling and verifiability; keep unresolved shared causes and shared contracts with one owner, freeze cross-slice contracts before parallel writes, and do not delegate when coordination cost exceeds the benefit. If the current route is not `SINGLE_OWNER` and the host allows it, dispatch the bounded Agents now and collect their Result Packets; do not stop at a routing proposal.
...
- Do not equate a complex task with mandatory multi-Agent execution. Use `$agent-orchestration` only after recording a concrete quality, critical-path, or context-isolation benefit; for an accepted cross-module program, keep the primary on user decisions, problem model, shared contracts, coordination, integration, acceptance and delivery instead of duplicating delegated leaf implementation. Default to bounded read-only parallelism, one write owner per shared artifact, delegation depth one, compact evidence-bearing Result Packets, and fresh escalation after one informative low-capability failure. Parallel writes require satisfied stage dependencies, frozen relevant shared contracts, disjoint write sets and slice-level acceptance; otherwise use a sequential handoff. Once a non-`SINGLE_OWNER` current route is selected and authorized, require an accepted host dispatch and terminal results; suggested roles or prompts are not execution.
...
6. Agent-orchestration RouteDecision, actual dispatch / terminal status or concrete blocker, assignments, write ownership, capability tiers and escalation boundary when delegation applies
...
7. If delegation has concrete value, use `$agent-orchestration` to establish an `OrchestrationProgram` for substantial multi-stage cross-module work, or one bounded current-stage route for simpler delegation. Keep only one current `RouteDecision`, issue bounded Assignment Capsules after the task model, stage dependencies and relevant shared-contract revisions are stable, then immediately dispatch and collect them when the host allows it; otherwise retain `SINGLE_OWNER` and record any host blocker
