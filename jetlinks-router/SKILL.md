...
- When Apache Commons utilities are already present or adjacent code already uses them, prefer them for object, collection, map, and array checks; do not introduce `org.apache.commons.lang3.StringUtils` in new or modified Java code. For Commons Lang string comparison/search/prefix/suffix/plain replace operations, follow `$jetlinks-conventions` and use `Strings.CS` / `Strings.CI` when the dependency provides `Strings`; if it does not, do not fall back to `StringUtils`, and route the version constraint or blank/empty/trim/default cases to `$jetlinks-conventions`.
