...
用于 JetLinks 前端页面开发、组件/hook/utils 能力复用、目录落点判断、状态管理与类型质量约束；坚持业务优先、参考为辅，默认沿用 Ant Design 风格；需要交互打磨时，可结合 `$frontend-design`，但仍以当前前端框架风格为准，且最终界面必须面向终端用户而非开发者。
...
- 使用 `$jetlinks-web` 在前端改造中优先复用 `@jetlinks-web-core/@jetlinks-web` 组件、hooks、utils，并按目录/状态/类型约束落地；先分析真实业务目标，做到业务优先、参考为辅，不要默认套后台 CRUD；视觉与组件语言默认沿用 Ant Design；如需交互或视觉优化，再结合 `$frontend-design`，但只能借鉴相似业务案例中真正适配的设计，且不要加入无意义的统计数字或装饰性数据块；线框图和设计说明只用于沟通，最终页面不要展示“交互方式”“设计原理”等开发者导向内容；若结构不确定，先问用户或先给线框图/效果图。
