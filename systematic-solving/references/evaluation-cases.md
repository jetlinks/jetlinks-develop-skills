...
| 实质复杂任务 | 因果不确定时比较真实候选；未决共享风险先明确相应契约和证据 | 按技术栈数量升级，或为已知根因虚构第二候选 |
| 已明确跨层能力 | 保留已接受方案，完成正常 retry、测试 mock 和多文件实现 | 仅因关键词或连续两次编辑强制重构或重新询问 |
...
| 取证数量诊断 | 默认一至两项，超额时诊断重复工作；用户或宿主显式资源上限才强制阻断 | 用默认次数否定有效证据，或越过明确授权的资源上限 |
| 无效观察修复诊断 | 默认集中修正一次；后续需有新增区分价值，引用本轮真实证据 | 只更换 decision identity 清零失败，或引用旧轮无效证据停止当前轮 |
...
| 首次实现仍失败 | 比较失败分区与签名；根因假设受支持但实现遗漏时补全同一方案，解释失效时再重构 | 未解释失败却扩大 if / fallback / mock / retry |
...
| 无效观察 | 检查在目标边界前因输入、环境、装配或测量方式失败时标记 `INVALID`，不改变目标解法；默认集中修正一次观察装置 | 把任意失败当成可用红灯并实施目标修复 |
| 无法区分的观察 | 观察到达目标边界但多个候选都预测相同结果时标记 `INCONCLUSIVE`，重设候选、边界或 discriminator | 原样重复或凭偏好选择方案 |
| 观察装置反复变化 | 同一假设和 discriminator 下连续更换工具、输入、提示、模拟或 rubric 时仍计为同一停滞面；重复修正应检查新增区分价值 | 通过换载体规避 Attempt 预算 |
...
- `action`：记录是否发生权威设计、候选 API 深化、生产实现、候选制品审查或普通运行态动作。设计 / 实现若声明 `evidence_ids`，每项必须存在且为 `DISCRIMINATING`；合法 USER resolution 不会把先前 `INVALID` / `INCONCLUSIVE` / `SCOPE_INVALID` 观察变成有效依据。

调查数量与重复修正默认输出 warnings；只有显式 `budget_policy.enforce_limit=true` 的资源预算才是数量硬限制。停止后的重复调查、无效授权和来源身份错误始终阻断。
