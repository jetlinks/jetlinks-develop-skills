...
普通新任务给出精确文件、符号或错误位置时，直接从锚点检索；不加载 `$task-continuity`，不创建任务账本、复合身份审计或代码图。实际恢复时才消费已有的原目标、已接受约束和 Next；当前源码查询已经足够时无需重新建立恢复协议。下文图身份与 query envelope 只约束图缓存复用或图内容注入，不是每条文本搜索的前置表单。
...
从当前问题实际需要的能力开始，已有可用工具可直接使用；只有缺少所需事实时再发现下一层：
...
- 无版本控制且确需复用缓存图：以工作区根、文件内容摘要、构建配置或任务 revision 建立 source fingerprint；实时精确查询不为此额外建账。
...
大仓库或 hub-heavy 结构中主动裁剪重复前向边、低置信邻居和不会改变下一动作的关系；不能因图已生成就全部注入上下文。索引和局部图必须绑定 source fingerprint；源码写入可只留下宿主运行态 dirty marker，直到一个通过任务相关性门禁的真实图查询再增量刷新受影响节点。不得在每次 Bash、每次编辑、session start 或上下文压缩后主动更新 / 重建整图。
