...
7. Pair with `$jetlinks-systematic-solving` when permission behavior spans several asset types / associations / callers, has competing ownership hypotheses, or a fix shifts the leak / denial to a sibling scenario. Do not accumulate exception branches for individual roles, assets, or callers.
