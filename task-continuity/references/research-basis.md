...
上述来源支撑的是分层状态、主动阶段压缩、identity-bound replay、结果复用和轨迹评测原则。`Contract / Checkpoint / DecisionState / Resume`、`READY / SNAPSHOT_REQUIRED / RESUME_AUDIT`、`previous_productive_action_id / pre_compaction_next_action_id / post_compaction_first_productive_action_id` 动作链、默认 3–7 个 anchors，以及第二 / 第三次恢复止空转门槛是基于这些原则和真实任务轨迹形成的工程协议，必须继续通过宿主轨迹调优，不能表述成论文直接规定的字段或常数。
