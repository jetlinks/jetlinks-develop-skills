...
5. Maintain two bounded logical views: a semantic Recovery Capsule for route, evidence, anchors, one executable next action, a cross-compaction resume fingerprint, and a loaded-rules ledger; and a machine Source Snapshot for source identity, expected changes, fingerprint strength, and missing layers. They may share one physical artifact, but neither may substitute for the other.
