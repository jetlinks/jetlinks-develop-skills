...
6. SQL 性能按真实场景验证
   ...
   - 只有本次改变查询形态、预期规模、索引、分页深度、并发、批量大小或资源风险时，才选择相称的执行计划、压力测试或等价性能验证；技术形态本身不触发压测。
   - 已确认风险需要压测但环境不可用时，说明替代验证和剩余风险；没有性能风险变化时不准备“不适用”证明。
...
- 查询形态、规模、索引、分页、并发或批量风险变化时，是否已有相称的执行计划、压力测试或等价性能验证
