...
- When Apache Commons utilities are already available in the target module or adjacent CRUD code, prefer them for object, collection, map, and array checks; do not introduce `org.apache.commons.lang3.StringUtils` in CRUD code. For string comparison/search/prefix/suffix/plain replace operations, follow `$jetlinks-conventions` and use `Strings.CS` / `Strings.CI` when the dependency provides `Strings`; if it does not, do not fall back to `StringUtils`, and use JDK, Spring, local helpers, or state the version constraint.
