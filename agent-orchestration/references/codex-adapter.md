...
- Treat the main Codex thread as a control-plane manager once a delegated program is active: it plans, dispatches, steers, accepts and requests host-level merge / validation, but does not author or repair source, tests, documentation or review findings. If integration needs code changes, stop the affected slice and dispatch a fresh leaf assignment.
...
- When the host can attach `orchestration_context` to `PreToolUse / PreDispatch`, pass the actor role, delegated-program state, action class, depth / delegation policy, exact requested / allowed / active write sets, contract state and semantic-fork status. The optional adapter can then deny high-confidence primary writes, leaf recursion, scope expansion and overlapping writes before execution. Missing context remains a no-op plus evaluator / prompt fallback; do not infer authority from command text.
- Preserve the host's stable event / tool-use identity. Repeated hook delivery for the same event must reuse its receipt, and acceptance / completion remains bound to the current task, optional run, workspace and contract revision rather than source fingerprint alone.
- Ask for `bounded_explorer` only after assigning the shared `decision_question`, one hypothesis, discriminator, evidence axis, read scope, and stop condition. Start with one or two in the default evidence round; treat higher counts as an efficiency diagnostic unless an explicit host limit applies, and stop after the primary records `FREEZE`, `ASK_USER`, or `BLOCKER`.
