...
7. Give every delegated slice an `Assignment Capsule`: assignment and parent identity, depth, objective and decision, allowed and excluded scope, inputs and source identity, acceptance signals and owner, output contract, budget and stop conditions, escalation triggers, permissions, and delegation authority. Treat every field as an upper bound: a child may narrow but never expand scope, write ownership, secrets, external effects, budget, or acceptance authority. For economy writes also record execution class, contract state, oracle, risk flags, and reversibility. One owner controls each writable artifact or shared state region.
...
Use [`scripts/evaluate_orchestration_trace.py`](scripts/evaluate_orchestration_trace.py) to check normalized orchestration traces for missing capsules, fan-out, overlapping writes, authority expansion, weak retry loops, evidence-free results, and absent integration. New adapters should emit `schema_version: 3`, which adds ancestry, delegation-authority and normalized scope / permission attenuation gates to the schema-version-2 dispatch-receipt and primary-acceptance gates; schema versions 1 and 2 remain readable for historical traces and are not evidence that those older traces enforced newer checks. The script accepts stdin or a JSON file, uses only the Python standard library, and never starts Agents or mutates task state.
