...
| 工具 | 推荐打法 | 更适合的任务 |
| --- | --- | --- |
| Claude Code / Codex | 给出目标、边界与验收；智能体按已有授权组织计划、实现与阶段验证，只有实质未决选择再确认 | 跨文件改动、批量修改、测试修复、交付整理 |
| Cursor | 建议分两轮：先找模块与相邻实现，再让它改代码和补测试 | 局部阅读、边写边审、人工介入较多的重构 |
| 共同要求 | 从有效锚点确认当前事实，按需计划与读取，复用已有授权，只做最小完整改动 | 所有 JetLinks 二开任务 |
...
2. 复杂任务先计划
   - 按当前动作选择 focused skill；复杂求解准入统一交给 `$systematic-solving`，机械跨层实现不自动升级。需要长任务主线、阶段状态或恢复时才加入 `$task-continuity`。
   - 计划只保留当前目标、持续约束、未完成阶段与下一动作；已有明确范围和实施授权直接推进，只有实质未决选择才询问。
   - 简单低风险小任务可在简短计划后直接实施。
3. 再发现
...
4. 再落地
   ...
   - 用 [systematic-solving Admission](systematic-solving/SKILL.md#admission) 判断是否进入系统诊断。一次失败先区分证据支持的实现缺口、机械装配与根因被否定；已明确的跨层实现、正常测试替身或已授权 retry 能力不自动升级。诊断中继续掩盖未解释失败或重复无信息检查时，才停止该循环并重新选择证据或假设
5. 再验证
...
- 提交已获授权时，在连贯阶段完成并集中验证后创建本地 commit，再更新真实 checkpoint；未授权提交时保留有效的阶段证据
- 上下文压缩或恢复后先核对 task ID / revision、branch / HEAD、changed paths，再只读取胶囊列出的少量锚点；不要重新扫描全仓
- 创建 PR 前核对总体验收与已有发布授权；满足时直接交付，只有确实未授权的发布才补充确认；中间共享沿授权维护同一个 draft
...
约束：
- 需要时用简短计划组织阶段；已有明确目标和授权直接实施，影响契约或权限的实质未决选择再问我
- 对复杂或高不确定性任务，先明确可观察目标、不变量、变化轴、竞争假设和能区分假设的证据；不要把计划本身当成问题分析
- 修复证据否定当前假设或反复扩展特例时，按 systematic-solving 的统一准入和停滞规则处理；已批准的 retry 能力、正常测试 mock 与阶段内编辑不自动升级
...
要求：
- 后端行为改变时复用或明确相应契约和测试目标；已有实施授权直接推进，仅实质未决选择再问。状态沿宿主已有 task / runtime 或 Trellis；其次使用已存在且验证忽略的位置，无安全载体时保留有界 active context，不自动创建目录或修改 Git 配置。长期稳定结论有变化再原位同步权威 docs
...
- 后端改动先明确改变的行为和验证目标；复用已有设计与授权，按风险决定必要的设计和用户决策，不因新功能或跨层自动落档、重批。
...
```text
这是一个 JetLinks 二开任务。请从当前工作区和有效锚点确认事实，选择最少相关 skills 完成目标，不猜模块名、包名或接口契约。沿用已有目标、约束和授权；进度提问与新提醒继续服务原主线，只有实质未决选择再问我。长任务状态使用已有安全载体，无载体时保留有界上下文，不自动创建运行态目录或改 Git 配置。实际恢复时按 task-continuity 核对必要身份，匹配后执行保存的下一动作；失配只对账相关范围。按连贯阶段和实际风险集中验证并复用有效证据；提交或发布已获授权时再执行对应操作。完成后说明实际改动、验证和真实风险，长期契约变化时原位同步权威文档。
```
