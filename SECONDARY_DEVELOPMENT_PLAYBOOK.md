...
2. 复杂任务先计划
   - 新功能、跨模块、前后端联动、接口 / 数据结构 / 事件变更、需求仍在变化或会拆成多个子任务的任务，先输出计划并等待确认，组合通用 `$systematic-solving` 建立可证伪的根因假设和按变化选择的验证范围，并用 `$task-continuity` 约束实时计划、恢复和阶段性交付，再加载 JetLinks router 扩展。
...
- 同一根因假设的一次实现仍未通过验收时，停止追加条件、fallback、retry、mock 或兼容分支；先说明新结果否定了什么，重建完整执行路径和按变化选择的验证范围后再继续
- 不要凭空假设模块名、包名、注解、Topic、命令 ID、表结构
- 保持与当前模块现有风格一致，只做最小必要改动
- 运行时校验只放在不可信输入、权威权限、状态 / 持久化不变量、危险操作或已确认错误转换的唯一 owner；DTO、框架或上游已保证时不重复，也不为异常测试或“更健壮”新增 guard
...
- 已确认契约要求的保存前不变量、唯一性冲突、资源不存在等用户可见异常，优先使用当前模块的 `i18nCode` / message key；不要为了补异常用例自行创造这些语义
...
要求：
- 如果这是较大的后端改动或新功能，先把待确认设计和测试目标写入宿主已有 task / runtime store 或 Trellis active task，把实时任务拆分写入非版本化 runtime / checkpoint；仅在 VCS 与 ignore 校验可用时使用经验证的 ignored artifact，无安全载体时保留有界 active-context capsule。确认后只把长期稳定结论原位提升到权威 docs
...
要求：
- 先扫描当前模块已有测试风格和相邻测试类
- 先识别本次新增 / 改变的可观察行为和 owning boundary；核心、边界、异常、权限、性能与集成按实际风险变化选择，不填固定清单
...
- 如果仓库已有覆盖率要求，补充覆盖率数据；如果没有，不为覆盖率数字增加无行为价值的测试
...
- 测试目标不能创造生产行为；框架、DTO、AssetsHolder 或上游契约已拥有的校验，不在 Controller、Service、Repository 多层复制。
...
- 测试要求必须写清楚，至少落到“改变了什么行为、由哪个边界证明、结果如何、还有什么真实风险没覆盖”。
